...
| Full Stack Codesign | `Project and Goals` | Drive |
| Current Projects | that tool's folder under `Build the Fab` | Drive |
| `stepper.html` timeline table | `Build the Fab/Maskless Lithography Stepper/Project Timeline` | Drive |
| Compute Kernel entry | `Design the Compute Kernel/` | Drive |
| Team — roles and roster | `Engineering Structure` | Drive |
| Colours, spacing, markup, nav | `style.css`, `index.html`, `stepper.html` | repo |
...
only by quietly disabling the sync while every run keeps reporting success.

Editing Drive avoids both: the change is in the authoritative copy, so every future sync
...
All output goes to `~/Library/Logs/mnfc-website-sync.log`, not to the terminal — the script
does `exec >>"$LOG" 2>&1` on line 17. Watch the tail of the log for the `RESULT:` line:

```bash
tail -f ~/Library/Logs/mnfc-website-sync.log
```

Or drive it interactively from Claude Code inside the repo:
...
- **Forgetting the footer date.** Any content change must update the `.synced` span in the
  `index.html` footer. `stepper.html` carries no `.synced` span — the date lives in one
  place only.
